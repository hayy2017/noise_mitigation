...
class LogisticRegressionMIML(LogisticRegression):
    ...
    def __init__(self, input_, n_in, n_out, W = None, b = None):
        ...
        """
        print 'MIML logistic'
        super(LogisticRegressionMIML, self).__init__( input_, n_in, n_out, W = None, b = None)

    def negative_log_likelihood_meanBatch(self, y):
        return -T.mean(T.log(self.p_y_given_x)[T.arange(y.shape[0]), y])

    def negative_log_likelihood_maxBatch(self, y):
        y_hat = self.p_y_given_x
        jj = T.argmax(y_hat, axis=0) # jj should be the indices of contexts with highest score (x, y_i)
        maxpredvec = y_hat[jj, T.arange(y_hat.shape[1])]
        y_ref = y[0]
        return -T.log(maxpredvec)[y_ref]
